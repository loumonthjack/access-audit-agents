 ...
 */
function simulateDeleteMutationCacheInvalidation(
 ...
 */
function simulateStartScanCacheInvalidation(
    queryClient: QueryClient
): { invalidatedQueries: string[] } {
    const invalidatedQueries: string[] = [];
...
describe('Property 21: Query Cache Invalidation on Mutation', () => {
    ...
    it('Property 21a: Delete mutation invalidates sessions cache', async () => {
        await fc.assert(
            fc.asyncProperty(
                paginatedSessionsArbitrary,
                async (sessionsResponse) => {
                    // Skip if no sessions
                    ...
                    const { invalidatedQueries } = simulateDeleteMutationCacheInvalidation(
                        queryClient
                    );

                    // Verify sessions.all was invalidated
                    expect(invalidatedQueries).toContain(
                        JSON.stringify(queryKeys.sessions.all)
                    );
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (existingSessions) => {
                    ...
                    const { invalidatedQueries } = simulateStartScanCacheInvalidation(
                        queryClient
                    );
            ...
            fc.asyncProperty(
                ...
                async (paginatedResponses) => {
                    ...
                    paginatedResponses.forEach((response, index) => {
                        queryClient.setQueryData(
                            queryKeys.sessions.list(index + 1),
                            response
                        );
                    });
    ...
    it('Property 21e: Reports cache is NOT affected by session mutations', async () => {
        await fc.assert(
            fc.asyncProperty(
                scanSessionArbitrary,
                fc.uuid(),
                async (session, reportId) => {
                    ...
                    const reportCachedBefore = queryClient.getQueryData(
                        queryKeys.reports.detail(reportId)
                    );
                    ...
                    const reportQueryState = queryClient.getQueryState(
                        queryKeys.reports.detail(reportId)
                    );
