...
function generateFilename(batchName: string | undefined, format: 'json' | 'html'): string {
  ...
  const baseName = batchName ? batchName.toLowerCase().replace(/[^a-z0-9]+/g, '-') : 'batch-report';
 ...
 * Requirements: 10.5
 */
export function BatchExportButtons({ batchId, batchName, className }: BatchExportButtonsProps) {
  ...
  return (
    <div className={clsx('space-y-2', className)} data-testid="batch-export-buttons">
      ...
      {/* Error message */}
      {error && (
        <div role="alert" className="text-sm text-error-600 bg-error-50 px-3 py-2 rounded-lg">
          {error}
        </div>
      )}
    </div>
