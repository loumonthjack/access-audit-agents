...
export function BatchProgress({
  ...
  const { batch, progress, currentPage, currentPageIndex, isConnected, error, isLoading } =
    useBatchProgress({
      batchId,
      enabled: true,
  ...
  return (
    <Card className="space-y-4" data-testid="batch-progress">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm font-medium text-neutral-600">{progressPercent}%</span>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
        ...
        <div>
          ...
          <span className="font-medium">
            {isFinished ? 'Done' : formatTimeRemaining(progress?.estimatedTimeRemaining ?? 0)}
          </span>
        </div>
      </div>
      ...
      {/* Error message */}
      {error && (
        <div role="alert" className="rounded-lg bg-error-50 border border-error-200 p-3">
