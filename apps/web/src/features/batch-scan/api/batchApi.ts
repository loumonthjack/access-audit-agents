...
import type { BatchSession, BatchReport } from '@/types/domain';
import type { SitemapParseRequest, SitemapParseResponse, CreateBatchScanRequest } from '../types';
 ...
 * Requirements: 10.5
 */
export async function exportBatchReport(batchId: string, format: 'json' | 'html'): Promise<Blob> {
 ...
 */
export async function listBatchScans(
  page: number = 1,
  limit: number = 10
): Promise<{
  data: BatchSession[];
 ...
 */
export function useBatchScanHistory({
  page = 1,
  limit = 10,
