...
export function ViolationList({
    ...
    const [expandedGroups, setExpandedGroups] = useState<Set<ImpactLevel>>(
        new Set(IMPACT_ORDER)
    );
    ...
    const groupedViolations = useMemo(
        () => groupByImpact(filteredViolations),
        [filteredViolations]
    );
            ...
            <div className="space-y-2 sm:space-y-3">
                {IMPACT_ORDER.map(impact => {
                    ...
                    return (
                        <div
                            key={impact}
                            className={clsx('rounded-lg border', config.bgColor)}
                        >
                            {/* Group header */}
                            <button
                            ...
                            >
                                <span className="flex items-center gap-2">
                                    ...
                                    <Badge variant={impact === 'critical' ? 'error' : impact === 'serious' ? 'warning' : 'default'}>
                                        {group.length}
                                    </Badge>
                                </span>
                                <svg
