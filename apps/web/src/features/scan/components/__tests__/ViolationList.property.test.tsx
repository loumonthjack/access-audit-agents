 ...
 */
const impactLevelArbitrary = fc.constantFrom<ImpactLevel>('critical', 'serious', 'moderate', 'minor');

/**
 * Arbitrary for generating violation status
 */
const violationStatusArbitrary = fc.constantFrom('pending' as const, 'processing' as const, 'fixed' as const, 'skipped' as const);
 ...
 */
const violationArbitrary = fc.integer({ min: 0, max: 999999 }).chain(index =>
    createViolationArbitrary(index)
);

/**
 * Arbitrary for generating a list of violations with unique IDs
 */
const violationsArbitrary = fc.integer({ min: 1, max: 20 }).chain(count => {
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (violations) => {
                    ...
                    const pendingFilterButton = screen.getByRole('button', { name: /pending/i, pressed: false });
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (violations) => {
                    ...
                    const skippedFilterButton = screen.getByRole('button', { name: /skipped/i, pressed: false });
