...
describe('ScanForm Property Tests', () => {
  ...
  it('Property 1: Valid URL triggers exactly one onSubmit call with correct parameters', async () => {
    await fc.assert(
      fc.asyncProperty(validUrlArbitrary, viewportArbitrary, async (generatedUrl, viewport) => {
        ...
        render(<ScanForm onSubmit={onSubmit} isLoading={false} />);
...
describe('ScanForm Loading State Property Tests', () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async () => {
          ...
          render(<ScanForm onSubmit={onSubmit} isLoading={true} />);
  ...
  it('Property 2b: Non-loading state allows form interaction', async () => {
    await fc.assert(
      fc.asyncProperty(validUrlArbitrary, async () => {
        cleanup();
  ...
  it('Property 3: Error prop displays non-empty error message', async () => {
    await fc.assert(
      fc.asyncProperty(errorMessageArbitrary, async (errorMessage) => {
        ...
        render(<ScanForm onSubmit={onSubmit} isLoading={false} error={errorMessage} />);
...
describe('ScanForm Button Disabled State Property Tests', () => {
  ...
  it('Property 4: Scan button is disabled when disabled prop is true', async () => {
    await fc.assert(
      fc.asyncProperty(fc.boolean(), async (isDisabled) => {
        cleanup();

        const onSubmit = vi.fn();

        render(<ScanForm onSubmit={onSubmit} isLoading={false} disabled={isDisabled} />);
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async () => {
          ...
          render(<ScanForm onSubmit={onSubmit} isLoading={false} disabled={true} />);
  ...
  it('Property 4c: Loading state disables button regardless of disabled prop', async () => {
    await fc.assert(
      fc.asyncProperty(fc.boolean(), async (disabledProp) => {
        cleanup();

        const onSubmit = vi.fn();

        render(<ScanForm onSubmit={onSubmit} isLoading={true} disabled={disabledProp} />);
