...
describe('Auth + Routes Integration Tests', () => {
  ...
  describe('useAuth Hook', () => {
    it('should provide auth context values', () => {
      const authenticatedContext = createAuthenticatedContext(mockUser);
      let capturedAuthValue: AuthContextValue | null = null;

      function TestComponent({ onAuth }: { onAuth: (auth: AuthContextValue) => void }) {
        const auth = useAuth();
        onAuth(auth);
        return <div>Test</div>;
      }
      ...
      render(
        <AuthContext.Provider value={authenticatedContext}>
          <TestComponent
            onAuth={(auth) => {
              capturedAuthValue = auth;
            }}
          />
        </AuthContext.Provider>
      );

      expect(capturedAuthValue).not.toBeNull();
      expect(capturedAuthValue!.user).toEqual(mockUser);
      expect(capturedAuthValue!.isAuthenticated).toBe(true);
      expect(capturedAuthValue!.isLoading).toBe(false);
    });
  ...
  describe('Auth Methods', () => {
    ...
    it('should call loginWithSSO method', async () => {
      ...
      function TestComponent() {
        ...
        return <button onClick={() => loginWithSSO('google')}>Login with Google</button>;
      }
  ...
  describe('useRequiresAuth Hook', () => {
    it('should return true in SaaS mode', () => {
      ...
      function TestComponent({ onRequiresAuth }: { onRequiresAuth: (value: boolean) => void }) {
        const requiresAuth = useRequiresAuth();
        onRequiresAuth(requiresAuth);
        return <div>Test</div>;
      }
      ...
      render(
        <AuthContext.Provider value={createMockAuthContext()}>
          <TestComponent
            onRequiresAuth={(value) => {
              capturedRequiresAuth = value;
            }}
          />
        </AuthContext.Provider>
      );
    ...
    it('should return true when requireAuthInSelfHosted is true', () => {
      ...
      function TestComponent({ onRequiresAuth }: { onRequiresAuth: (value: boolean) => void }) {
        const requiresAuth = useRequiresAuth(true);
        onRequiresAuth(requiresAuth);
        return <div>Test</div>;
      }
