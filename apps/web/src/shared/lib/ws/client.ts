...
export class WebSocketClient {
    ...
    // Event handlers
    private clientEventHandlers = new Map<WSClientEvent['type'], Set<WSEventHandler<WSClientEvent['type']>>>();
    private progressEventHandlers = new Map<ProgressEvent['type'], Set<ProgressEventHandler<ProgressEvent['type']>>>();
     ...
     */
    onClientEvent<T extends WSClientEvent['type']>(
        type: T,
        ...
        this.clientEventHandlers.get(type)!.add(handler as unknown as WSEventHandler<WSClientEvent['type']>);

        // Return unsubscribe function
        return () => {
            this.clientEventHandlers.get(type)?.delete(handler as unknown as WSEventHandler<WSClientEvent['type']>);
        };
    }
    ...
    onProgressEvent<T extends ProgressEvent['type']>(
        ...
        this.progressEventHandlers.get(type)!.add(handler as unknown as ProgressEventHandler<ProgressEvent['type']>);

        // Return unsubscribe function
        return () => {
            this.progressEventHandlers.get(type)?.delete(handler as unknown as ProgressEventHandler<ProgressEvent['type']>);
        };
    }
